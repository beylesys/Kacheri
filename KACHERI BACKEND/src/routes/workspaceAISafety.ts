...
export default async function workspaceAISafetyRoutes(app: FastifyInstance) {
  app.get<{
    ...
    const exportRow = await db.queryOne<{ total: number; verified: number }>(
      `
      SELECT
        COUNT(*) as total,
        SUM(CASE
    ...
    const verificationRate =
      exportRow && exportRow.total > 0
        ? Math.round((exportRow.verified / exportRow.total) * 100)
        : 100;

    // Calculate determinism rate from compose actions
    const composeRow = await db.queryOne<{ total: number; pass: number; drift: number }>(
      `
      SELECT
        COUNT(*) as total,
        SUM(CASE
    ...
    const determinismRate =
      composeRow && composeRow.pass + composeRow.drift > 0
    ...
    const healthRows = await db.queryAll<{
      docId: string;
    ...
    const recentRows = await db.queryAll<{
      id: number;
      docId: string;
    ...
    const providerRows = await db.queryAll<{
    ...
    return {
      ...
      summary: {
        totalDocs: summaryRow?.totalDocs ?? 0,
        docsWithAI: summaryRow?.docsWithAI ?? 0,
        totalAIActions: summaryRow?.totalAIActions ?? 0,
        verificationRate,
        determinismRate,
      },
      health: {
